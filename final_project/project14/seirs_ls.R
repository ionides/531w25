...
pf[[1]] |> coef() |> bind_rows() |>
  ...
  write_csv("influenza_params.csv")
...
local_mifs_seirs <- bake(file = "seirs_ls.RDS", {
  local_mifs_seirs <- foreach(i = 1:Nlocal, .combine = c, .packages = c("pomp", "tidyverse")) %dopar% {
    seirs_pomp %>% 
      mif2(
        Np = Np,
        Nmif = Nmif,
...
local_ll_seirs <- bake(file = "seirs_results.RDS", {
  local_ll_seirs <- foreach(mf = local_mifs_seirs, .combine = rbind, .packages = c("pomp", "tidyverse")) %dopar% {
    ll_vals <- replicate(Nreps_eval, logLik(pfilter(mf, Np = Np)))
