...
- A **time series** is a sequence of numbers, called **data**. In general, we will suppose that there are $N$ numbers, $y^*_1, y^*_2, \dots, y^*_N$, collected at an increasing sequence of times, $t_1, t_2, \dots, t_N$.
...
- Scientifically, we consider the hypothesis that $y^*_{1:N}$ is well described as a realization of $Y_{1:N}$ for some unknown value of $\theta$.
...
- Perhaps unfortunately, $\hat\mu_n$ denotes both the estimator function and its value evaluated at $y^*_{1:N}$. This is standard terminology for data analysis.
...
- $\hat\mu=\hat\mu(y^*_{1:N})$ is the **sample mean**. 

- We can compute $\hat\mu$ for any dataset. It is only a reasonable estimator of the mean function when a mean stationary model is appropriate.
