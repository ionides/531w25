...
# Introduction to business cycles and mortality
...
- Procyclical mortality, if it exists, shows that a key measure of population health is worse in economic booms than in recessions.
...
## A time series of life expectancy in the USA {.fragile}

```{python}
#| echo: true
e_data = pd.read_csv("life_expectancy_usa.csv", sep='\s+',
...
- `e0f` and `e0m` are LEB for females and males, but we focus on `e0`.
...
- LEB is a standard way to combine all the age-specific mortality rates into a single number.
...
## A time plot of the raw data {.fragile}
...
## The Hodrick-Prescott filter {.fragile}
...
## A regression with ARMA errors model {.fragile}

- We can investigate the dependence of $e^{HP}_{1:N}$ on $u^{HP}_{1:N}$ using a regression with ARMA errors model,
\begin{equation}
...
```{python}
#| echo: true
# Reshape exog to 2D with column name for proper parameter naming
import pandas as pd
u_hp_df = pd.DataFrame(u_hp_cycle, columns=['u_hp'])
a0 = SARIMAX(e_hp_cycle, exog=u_hp_df, order=(1, 0, 0)).fit()
```
...
print(f"Coefficient of u_hp: {u_hp_coef:.6f}")
print(f"Standard error:      {u_hp_se:.6f}")
print(f"AR(1) coefficient:   {ar1_coef:.4f}")
print(f"Residual variance:   {a0.scale:.6f}")
...
```{python}
#| echo: true
a0_null = SARIMAX(e_hp_cycle, order=(1, 0, 0)).fit(disp=False)
...
- We have clear statistical evidence for a positive association between detrended unemployment and detrended life expectancy.
...
# Looking for dependence in bivariate time series
...
ccf_values = np.correlate(e_hp_cycle - e_hp_cycle.mean(),
...
plt.axhline(y=1.96/np.sqrt(len(e_hp_cycle)), color='b',
...
- The strong positive cross-correlation at lag zero supports pro-cyclical mortality.

- The oscillatory pattern is not significant pointwise but might be more evident in the frequency domain.
...
## Coherency, coherence and phase {.fragile}

- The *coherency* is the normalized cross-spectrum,
\begin{equation}
...
freqs, Pxx = signal.welch(e_hp_cycle, nperseg=len(e_hp_cycle)//4)
freqs, Pyy = signal.welch(u_hp_cycle, nperseg=len(u_hp_cycle)//4)
freqs, Pxy = signal.csd(e_hp_cycle, u_hp_cycle,
                        nperseg=len(e_hp_cycle)//4)
...
- Python calculates squared coherence, the squared absolute coherency. This is like reporting $r^2$ for regression, rather than $|r|$.
...
## Explaining the choice of an ARMA(1,0) error model {.fragile}

- Model selection by AIC for regression with ARMA errors follows the same approach as for ARMA models.
...
def aic_table(data, P, Q, xreg=None):
    ...
    for p in range(P+1):
        for q in range(Q+1):
            try:
                if xreg is not None:
                    model = SARIMAX(data, exog=xreg,
                                    order=(p, 0, q)).fit(disp=False)
                else:
                    model = SARIMAX(data, order=(p, 0, q)).fit(disp=False)
                table[p, q] = model.aic
            except:
                table[p, q] = np.nan
...
```{python}
#| echo: true
a3 = SARIMAX(e_hp_cycle, exog=u_hp_df, order=(3, 0, 1)).fit()
```
...
print(f"Residual variance:   {a3.scale:.6f}")
print(f"Log likelihood:      {a3.llf:.2f}")
```

##  {.fragile}

- The estimated ARMA(3,1) is at the boundary of invertibility, with an MA1 coefficient on the unit circle.
...
## Consistency through time {.fragile}
...
## Analysis of temporal differences {.fragile}
...
print(f"Residual variance:   {a4.scale:.6f}")
