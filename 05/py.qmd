...
```{python}
#| echo: true
print(f"Chi-squared 95% cutoff: {stats.chi2.ppf(0.95, df=1):.3f}")
```
...
```{python}
#| echo: true
ma_params = huron_arma21.maparams
ma_poly = np.r_[1, ma_params]
MA_roots = np.roots(ma_poly)
print(f"MA root: {MA_roots[0]:.3f}")
print(f"MA root magnitude: {np.abs(MA_roots[0]):.3f}")
```
...
- Do we have a non-invertibility problem? We investigate this using profile and bootstrap methods. The claimed standard error on the MA1 coefficient, from the Fisher information approach, is small.
...
- To do this, we need to maximize the ARMA likelihood while fixing the MA1 coefficient at a range of values.
...
for k in range(K):
    try:
        ...
        # Set starting values and constrain MA1
        start_params = huron_arma21.params.copy()
        start_params[3] = ma1_vals[k]  # MA1 is index 3
        results = model.fit(start_params=start_params, method='nm')
        profile_loglik[k] = results.llf
    except:
        profile_loglik[k] = np.nan
...
#| fig-width: 9
#| fig-height: 4
K = 100  # Reduced for computational efficiency
ma1_vals = np.linspace(0.2, 1.05, K)
profile_loglik = np.zeros(K)

# Note: Exact profile likelihood with fixed parameters is complex in statsmodels
# This is an approximate approach
for k in range(K):
    try:
        model = ARIMA(huron_level, order=(2,0,1))
        # Use different starting values centered on ma1_vals[k]
        start = huron_arma21.params.copy()
        start[3] = ma1_vals[k]
        results = model.fit(start_params=start, method='bfgs', maxiter=5)
        profile_loglik[k] = results.llf
    except:
        profile_loglik[k] = np.nan
...
ar = params[:2]  # AR parameters
ma = params[2:3]  # MA parameter
intercept = params[3]
...
for j in range(J):
    try:
        ...
        Y_j = arma_process.generate_sample(nsample=len(huron_level),
                                            scale=sigma) + intercept
        # Fit model
        model_j = ARIMA(Y_j, order=(2,0,1)).fit()
        theta_list.append(model_j.params[2])  # MA1 coefficient
    except:
        pass
...
#| fig-height: 4
np.random.seed(578922)
J = 200  # Reduced for compilation speed
params = huron_arma21.params
ar = params[:2]
ma = params[2:3]
intercept = params[3]
...
for j in range(J):
    try:
        ...
        Y_j = arma_process.generate_sample(nsample=len(huron_level),
                                            scale=sigma) + intercept
        model_j = ARIMA(Y_j, order=(2,0,1)).fit()
        theta_list.append(model_j.params[2])
    except:
        pass
...
#| eval: false
plt.figure(figsize=(9, 4))
plt.hist(theta_ma1, bins=30, density=True, alpha=0.7, edgecolor='black')
plt.xlabel('ma1')
plt.ylabel('Density')
plt.tight_layout()
plt.show()
...
```{python}
#| echo: true
print(f"Range of ma1 estimates: [{theta_ma1.min():.3f}, {theta_ma1.max():.3f}]")
```
...
**Question.** How would you refine this density estimation procedure to respect the range of the parameter estimation procedure?
