...
# Stationary causal linear processes

## Definition: Stationary causal linear process

- A **stationary causal linear process** is a time series model that can be written as
...
- We assumed we can move $\sum_{j=0}^\infty \sum_{k=0}^\infty$ through $\mathrm{Cov}$, discussed below.
...
### Convergence of the infinite sums in M8.1 and M8.2
...
- Write $\phi(x)= 1-\phi_1 x -\phi_2 x^2 -\dots -\phi_p x^p$, an order $p$ polynomial. The equation M1 for the AR(p) model can be rearranged to give
\begin{equation}
...
[M9] $\quad Y_n = \phi_1 Y_{n-1}+\phi_2Y_{n-2}+\dots+\phi_pY_{n-p} + \epsilon_n +\theta_1 \epsilon_{n-1} +\dots+\theta_q\epsilon_{n-q}$,
...
- The general stationary ARMA(p,q) also has a mean $\mu$ so we get
...
- We have seen the ARMA model written as a ratio of two polynomials,
\begin{equation}
Y_n = \frac{\theta(B)}{\phi(B)} \epsilon_n.
\end{equation}
...
## The deterministic skeleton: Using differential equations to study ARMA models
...
- We have to attend to the considerable amount of randomness (unpredictability) present in data and systems we want to study.
...
- Rather than deriving a deterministic skeleton from a stochastic time series model, we can instead add stochasticity to a deterministic model to get a model that can explain non-deterministic phenomena.
