...
- SARMA is a special case of ARMA, where the AR and MA polynomials are factored into a **monthly** polynomial in $B$ and an **annual polynomial** (also called **seasonal polynomial**) in $B^{12}$.
...
- One could write a SARMA model for some period other than 12.
...
plt.figure(figsize=(9, 3))
plt.plot(time, huron_level, '-')
plt.axvline(x=2014.99, color='red')
plt.xlabel('Year')
plt.ylabel('Lake level (m)')
plt.tight_layout()
plt.show()
...
- As well as the $p+q+1$ parameters in the ARMA(p,q) model, there will usually be unknown parameters in the mean function.

## Linear regression with ARMA errors

- When the mean function (also known as the trend) has a linear specification,
\begin{equation}
...
```{python}
#| echo: true
fit0 = SARIMAX(huron_jan, order=(1, 0, 0)).fit()
```
...
- Construct two different tests using the Python output above.
...
# Testing for trend vs unit root

## Augmented Dickey-Fuller test

- A time series model is said to have a **unit root** if its first difference is stationary.
...
- ADF test is sometimes erroneously called a test for "stationarity". However, many non-stationary models remain non-stationary after any amount of differencing. For example, models with non-polynomial trend.
