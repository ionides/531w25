...
**Question.** A coding exercise: Explain how the tabulated data in `U1` are converted to a time series, below.
...
#| fig-width: 10
#| fig-height: 5
u1 = U1.iloc[:, 1:13].values.T.flatten()
...
```{python}
#| echo: true
U2 = pd.read_csv("adjusted_unemployment.csv", comment='#')
u2 = U2.iloc[:, 1:13].values.T.flatten()
```
...
- To help understand the seasonal adjustment, we look at what it does to the smoothed periodogram.

- We need to compute smoothed periodograms for both time series and compare them.
...
#| fig-width: 11
#| fig-height: 5
# Compute smoothed periodograms using Welch's method
freqs1, psd1 = signal.welch(u1, fs=12, nperseg=len(u1)//8,
                             scaling='spectrum')
freqs2, psd2 = signal.welch(u2, fs=12, nperseg=len(u2)//8,
                             scaling='spectrum')
...
- Note: For a report, we should add units to plots. Extra details (like `bandwidth` in the periodogram plot) should be explained or removed.
...
- The ratio of the periodograms of the smoothed and unsmoothed time series is the *frequency response* of the smoother.
...
- Smoothers are linear and time invariant, at least approximately. If we scale or shift the data, we expect the smoothed estimate to have the same scale or shift. We expect a smooth approximation to the sum of two time series to be approximately the sum of the two smoothed series.

## Calculating a frequency response function {.fragile}

- We investigate the frequency response of the smoother used by Bureau of Labor Statistics to deseasonalize the unemployment data.

```{python}
#| echo: false
# Compute unsmoothed periodograms
freqs_u1, psd_u1 = signal.periodogram(u1, fs=12, scaling='spectrum')
freqs_u2, psd_u2 = signal.periodogram(u2, fs=12, scaling='spectrum')
```

- We find the parts that we need to plot the frequency response.

```{python}
#| echo: true
#| eval: false
# The frequency response is the ratio of the two spectra
...
## Estimating trend by Loess smoothing {.fragile}
...
## A Loess smooth of unemployment {.fragile}
...
#| fig-width: 10
#| fig-height: 4
# lowess returns [x, y] so we extract the smoothed values
u1_lowess = lowess(u1, date, frac=0.5)
...
freqs_raw, psd_raw = signal.periodogram(u1, fs=12, scaling='spectrum')
...
## Extracting business cycles: A band pass filter {.fragile}
...
#| echo: true
#| eval: false
u_low = lowess(u1, date, frac=0.5)[:, 1]
u_hi = u1 - lowess(u1, date, frac=0.1)[:, 1]
...
## Business cycles in detrended economic data {.fragile}
...
freqs_cyc, psd_cyc = signal.welch(u_cycles, fs=12, nperseg=len(u_cycles)//20,
                                    scaling='spectrum')
freqs_u1_welch, psd_u1_welch = signal.welch(u1, fs=12, nperseg=len(u1)//20,
                                              scaling='spectrum')
...
if np.any(hi):
    ...
    if len(hi_indices) > 0:
        ...
        print(f"Frequency range (ratio > 0.5): [{l_interp:.3f}, {r_interp:.3f}]")
```
...
**Question.** Describe the frequencies (and corresponding periods) that this decomposition identifies as business cycles. Note: units of frequency are omitted to give you an exercise!
...
#| fig-width: 9
#| fig-height: 4
freqs_zoom, psd_zoom = signal.welch(u1, fs=12, nperseg=len(u1)//40,
                                     scaling='spectrum')
...
## Common smoothers in Python {.fragile}
...
## Bandwidth for a smoother {.fragile}
