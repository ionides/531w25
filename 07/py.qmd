...
- We may work with **normalized** eigenvectors that are scaled such that $u_n^\mathsf{T} u_n = 1$.

----

- We can also check that the components of $Y$ in the directions of different eigenvectors are uncorrelated.
...
for i in range(5):
    ax1.plot(V_eigenvectors[:, i])
for i in range(5, 9):
    ax2.plot(V_eigenvectors[:, i])
plt.tight_layout()
plt.show()
```

## Eigenvalues for the covariance matrix of an AR(1) model with $N=100$ and $\phi=0.8$
...
- We look for a sinusoidal solution, $u_n = e^{2\pi i\omega n}$, where $\omega$ is cycles per unit time.
...
\begin{eqnarray*}
d_n &=& \frac{1}{\sqrt{N}} \sum_{k=1}^N y_k e^{-2\pi i k n/N}
\\
&=& c_n - i s_n
\end{eqnarray*}
...
- `fft` is an implementation of the fast Fourier transform algorithm, which enables computation of all the frequency components with order $N\log(N)$ computation. Computing the frequency components may appear to require a matrix multiplication involving order $N^3$ additions and multiplications. When $N=10^5$ or $N=10^6$ this difference becomes important!
...
plt.title('Unsmoothed periodogram')
plt.tight_layout()
plt.show()
```
...
freqs_w, psd_w = welch(low, nperseg=len(low)//4,
                       scaling='spectrum')
plt.figure(figsize=(10, 4))
plt.semilogy(freqs_w, psd_w)
plt.xlabel('Frequency [cycles per observation]')
plt.ylabel('Power spectral density')
plt.title('Smoothed periodogram (Welch method)')
plt.ylim([15, 100])
plt.tight_layout()
plt.show()
```
...
#| fig-height: 4
from scipy.signal import welch
freqs_w, psd_w = welch(low, nperseg=len(low)//4, scaling='spectrum')
plt.figure(figsize=(10, 4))
plt.semilogy(freqs_w, psd_w)
plt.xlabel('Frequency [cycles per observation]')
plt.ylabel('Power spectral density')
...
**Question.** What is the default periodogram smoother in Python's scipy?
...
**Question.** Why is that default chosen?

## More details on computing and smoothing the periodogram

- In R, typing `?spectrum` leads to `?spec.pgram`.

- By default, R removes a linear trend, fitted by least squares. This may often be a sensible thing to do. Why?

- R then multiplies the data by a quantity called a **taper**, computed by `spec.taper`.
...
The default taper in R smooths the first and last $p=0.1$ fraction of the time points, by modifying the detrended data $y_{1:N}$ to tapered version $z_{1:N}$ defined by
\begin{equation} \nonumber
z_n =
  \left\{
  \begin{array}{ll}
    y_n (1-\cos(\pi n/Np))/2 & \text{ if } 1\le n< Np \\
    y_n  & \text{ if } Np \le n \le N(1-p) \\
    y_n (1-\cos(\pi [N+1-n]/Np))/2 & \text{ if } N(1-p)<n\le N
  \end{array}\right.
\end{equation}
...
#| echo: true
#| eval: false
# Create R-style taper
N = 100
p = 0.1
taper = np.ones(N)
n_taper = int(N * p)
for i in range(n_taper):
    taper[i] = (1 - np.cos(np.pi * (i+1) / (N*p))) / 2
    taper[N-1-i] = (1 - np.cos(np.pi * (i+1) / (N*p))) / 2
plt.figure(figsize=(8, 2.5))
plt.plot(taper)
plt.title('Default taper in R, for time series of length 100')
plt.axvline(x=10, linestyle='dotted', color='red')
plt.axvline(x=90, linestyle='dotted', color='red')
plt.tight_layout()
plt.show()
```
...
# Fit AR model - try different orders and pick best AIC
aic_values = []
orders = range(1, min(21, len(low)//5))
for order in orders:
    try:
        model = AutoReg(low, lags=order, old_names=False).fit()
        aic_values.append(model.aic)
    except:
        aic_values.append(np.inf)

best_order = orders[np.argmin(aic_values)]
ar_model = AutoReg(low, lags=best_order, old_names=False).fit()

# Compute spectrum from AR model
ar_coefs = -ar_model.params[1:]
freqs_ar = np.linspace(0, 0.5, 1000)
spectrum_ar = ar_model.scale / np.abs(
    1 - np.sum([ar_coefs[i] * np.exp(-2j*np.pi*freqs_ar*(i+1))
                for i in range(len(ar_coefs))], axis=0))**2
...
plt.xlabel('Frequency [cycles per observation]')
plt.ylabel('Spectral density')
plt.title(f'Spectrum estimated via AR({best_order}) model picked by AIC')
plt.tight_layout()
plt.show()
```
