...
- You can get this file from the [course repository on GitHub](https://github.com/ionides/531w25).

- Better, you can make a local clone of this git repository that will give you an up-to-date copy of all the data, notes, code, homeworks and solutions for this course.
...
$$\hat\mu_1= \frac{1}{N}\sum_{n=1}^N y_n, \hspace{2cm}
...
Another basic thing to do is to fit an **autoregressive-moving average** (ARMA) model. We'll look at ARMA models in much more detail later in the course. Let's fit an ARMA model given by
...
We model $\epsilon_1\dots,\epsilon_N$ to be an independent, identically distributed (i.i.d.) sequence. To be concrete, let's specify a model where they are normally distributed with mean zero and variance $\sigma^2$.
...
Some exploration is required to extract the quantities of primary interest from the fitted ARMA model in Python.
...
# Comparing the i.i.d. estimate with the ARMA estimate
...
- In this case, the two estimates, $\hat\mu_1=$ `{python} f"{mu1:.2f}"` and $\hat\mu_2=$ `{python} f"{mu2:.2f}"`, and their standard errors, $SE_1=$ `{python} f"{se1:.2f}"` and $SE_2=$ `{python} f"{se2:.2f}"`, are close.

- For data up to `{python} int(yr_max)`, $\hat\mu_1^{`{python} int(yr_max)`}=$ `{python} f"{mu1_subset:.2f}"` and $\hat\mu_2^{`{python} int(yr_max)`}=$ `{python} f"{mu2_subset:.2f}"`, with standard errors, $SE_1^{`{python} int(yr_max)`}=$ `{python} f"{se1_subset:.2f}"` and $SE_2^{`{python} int(yr_max)`}=$ `{python} f"{se2_subset:.2f}"`.
...
We see slow variation in the residuals, over a decadal time scale. However, the residuals $r_{1:N}$ are close to uncorrelated. We see this by plotting their pairwise sample correlations at a range of lags. This is the **sample autocorrelation function**, or **sample ACF**, written for each lag $h$ as
...
- $\{\epsilon_n\}$ is i.i.d. $N(0,1)$, $\{\nu_n\}$ is i.i.d. $N(0,\sigma_{\nu}^2)$, $\{\omega_n\}$ is i.i.d. $N(0,\sigma_\omega^2)$.

- $H_n$ is unobserved volatility at time $t_n$. We only observe the return, modeled by $Y_n$.
