...
- A time series model which is both mean stationary and covariance stationary is **weakly stationary** or **second order stationary**.

- A time series model for which all joint distributions are invariant to shifts in time is **strongly stationary** or **strictly stationary**.
...
## Sinusoidal white noise visualization
...
# Building models using white noise

## Using white noise to build time series models
...
One approach is to use the ARIMA simulation functionality in statsmodels.
...
Note: The simulations may differ slightly from R's arima.sim due to different initialization methods.
...
We simulate M4 using similar methods as for the autoregressive model.
...
```{python}
#| echo: true
print(f"X1 and X2 close: {np.allclose(X1, X2)}")
```
...
# Modeling financial returns using white noise
